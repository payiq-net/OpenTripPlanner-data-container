...
const buildGraph = function (config) {
  ...
  const p = new Promise((resolve, reject) => {
    ...
    const buildGraph = exec(`docker run -v ${hostDataDir}/build:/opt/opentripplanner/graphs -v ${process.cwd()}/logback-include-extensions.xml:/opt/opentripplanner/logback-include-extensions.xml --rm --entrypoint /bin/bash hsldevcom/opentripplanner:${graphBuildTag}  -c "java -Xmx9g -jar otp-shaded.jar --build --save ./graphs/${config.id}/router"`, { maxBuffer: constants.BUFFER_SIZE })
