...
const buildGraph = function (router) {
  ...
  return new Promise((resolve, reject) => {
    ...
    const buildGraph = exec(`docker run -v ${hostDataDir}/build:/opt/opentripplanner/graphs --mount type=bind,source=/Users/vesameskanen/digitransit/otp/logback-include-extensions.xml,target=/opt/opentripplanner/logback-include-extensions.xml -e ROUTER_NAME=${process.env.ROUTER_NAME} --rm --entrypoint /bin/bash hsldevcom/opentripplanner:${graphBuildTag}  -c "java ${JAVA_OPTS} -jar otp-shaded.jar --build --save ./graphs/${router.id}/router"`, { maxBuffer: constants.BUFFER_SIZE })
...
module.exports = {
  buildOTPGraphTask: router => buildGraph(router)
    .then((commit, router) => packData(commit, router))
